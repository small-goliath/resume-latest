...
**학력 (`NEXT_PUBLIC_EDUCATION`)**: JSON 배열 형식으로, 각 학력 사항을 객체로 표현합니다.
```json
[
...
**보유 기술 (`NEXT_PUBLIC_SKILLS`)**: JSON 배열 형식으로, 기술 분류를 객체로 표현합니다.
```json
[
...
**수상 내역 (`NEXT_PUBLIC_AWARDS`)**: JSON 배열 형식으로, 각 수상 내역을 객체로 표현합니다.
```json
[
  {
    "awardName": "우수상",
    "category": "xxxx 경진대회",
    "certificateImageUrl": "https://cdn.sample.com/certificate.png",
    "link": "https://example.com/award-details"
  }
]
```
- `certificateImageUrl` (선택): 상장 이미지 URL
- `link` (선택): 수상 내역 상세 링크

**봉사활동 (`NEXT_PUBLIC_VOLUNTEERING`)**: JSON 배열 형식으로, 각 봉사활동을 객체로 표현합니다.
```json
[
...
**대외활동 (`NEXT_PUBLIC_ACTIVITIES`)**: JSON 배열 형식으로, 각 대외활동을 객체로 표현합니다.
```json
[
...
**인턴십 (`NEXT_PUBLIC_INTERNSHIPS`)**: JSON 배열 형식으로, 각 인턴십 경험을 객체로 표현합니다.
```json
[
...
**연구활동 (`NEXT_PUBLIC_RESEARCH`)**: JSON 배열 형식으로, 각 연구활동을 객체로 표현합니다.
```json
[
  {
    "category": "학술논문",
...
**동료 평가 (`NEXT_PUBLIC_PEER_REVIEWS`)**: JSON 배열 형식으로, 각 동료 평가 이미지 URL을 문자열로 포함합니다.
```json
[
...
**사이드 프로젝트 (`NEXT_PUBLIC_SIDE_PROJECTS`)**: JSON 배열 형식으로, 각 사이드 프로젝트를 갹체로 표현합니다.
```json
[
  {
    "title": "Autogram",
...
**경력 타임라인 (`NEXT_PUBLIC_FOOTPRINTS`)**: JSON 배열 형식으로, 각 경력을 객체로 표현합니다.
```json
[
